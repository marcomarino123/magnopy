...
to the :math:`\boldsymbol{\hat{n}}` unit vector

.. math::
   \boldsymbol{S_i} = S_i\,\boldsymbol{R_i}\,\boldsymbol{\hat{n}}
...
.. math::
  ...
  =
  \boldsymbol{R_m}(\boldsymbol{q})\,\boldsymbol{R_i}

Magnopy chooses to absorve the intra-cell spiral contribution inside the
angle
:math:`\phi_i(\boldsymbol{q})=\boldsymbol{q}\cdot\boldsymbol{r_i}+\Delta\,\phi_i`

The single-:math:`\boldsymbol{q}` spin state is generated by first placing a
unit vector :math:`\boldsymbol{\hat{n}}` at each atomic site, and subsequently
rotating it (and multiplying by :math:`S_i`)
