...
@given(ARRAY, ARRAY)
def test_energy_p21(parameter1, parameter2):
    ...
    convention = Convention(multiple_counting=True, spin_normalized=False, c21=1)
...
@given(ARRAY)
def test_gradient_p21(parameter):
    parameter = (parameter + parameter.T) / 2
    cell = np.eye(3, dtype=float)
