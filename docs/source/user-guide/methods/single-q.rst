...
2.  These vectors are subsequently rotated with rotation matrices
    ...
    This matrix was introduced :ref:`previously <user-guide_methods_spin-rotations>`,
    and it  does not depend on the position of the unit cell in the lattice (i.e. it
    does not depend on index :math:`m`), but in general depends on the spiral vector
    :math:`\boldsymbol{q}`:
