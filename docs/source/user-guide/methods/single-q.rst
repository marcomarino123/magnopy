...
================================
Magnopy construct the single-q conical state by populating all the atomic sites with the
collinear spin vectors :math:`\boldsymbol{S^F_{mi}}`, that are oriented along the direction of
the vector :math:`\boldsymbol{n}`:

.. math::
  \boldsymbol{S^F}_{mi}=S_i\,\boldsymbol{\hat{n}}

and subsequently rotating them first with the rotation :math:`\boldsymbol{R}_i`, which does not
depend on the position of the unit cell in the lattice and then with the conical rotation
:math:`\boldsymbol{R}_m`, which does not depend on the position of the
atoms within unit cell:

.. include:: repeated-formulas/spin-from-ferro-any.inc

The rotation :math:`\boldsymbol{R}_i` populates each unit cell by a collection of non-collinear
atomic spins. Because :math:`\boldsymbol{R}_i` does not depend on :math:`m`, all unit
cells are replica of each other. The rotation :math:`\boldsymbol{R}_m`
generates the cone spin arrangement among unit cells.

Intra-cell spin generation
==========================
Magnopy lets each intra-cell atomic spins :math:`\boldsymbol{S}_i`
have an independent orientation, that is described by the angles
:math:`(\theta_i,\,\phi_i)` measured in the :math:`(\,u,\,v,\,n\,)` reference
frame. This is,

.. math::
   \boldsymbol{S}_i = \boldsymbol{R}_i\,\boldsymbol{S^F}_i=S_i \,\boldsymbol{\hat{f}}_i
...
.. math::
  \boldsymbol{R_{r(\phi_i)}}(\theta_i)=\boldsymbol{R_{r}}(\theta_i,\phi_i)=e^{\boldsymbol{\hat{r}}(\phi_i) \theta_i\times}=
  (\boldsymbol{\hat{p}}_i\,\boldsymbol{\hat{t}}_i\,\boldsymbol{\hat{f}}_i)

and defines the reference frame :math:`(\,p_i\,t_i\,f_i\,)`,
as has been described in the section :ref:`user-guide_methods_spin-rotations`.


Cone direction, spiral vector and rotation matrix
================================================
The single-:math:`\boldsymbol{q}` conical spiral state is described by the
unit vector :math:`\boldsymbol{\hat{n}}` that defines the cone axis direction,
and by the spiral vector :math:`\boldsymbol{q}` that defines the cone propagation.
Both vectors define the rotation matrix
:math:`\boldsymbol{R}_m=\boldsymbol{R_n}(\phi_m)=e^{\boldsymbol{\hat{n}}\,\phi_m\,\times}`,
where the azimut angle is

.. math::
  \phi_m = \boldsymbol{q}\cdot\boldsymbol{r_m}

where the explicit expression for the matrix is
...
.. math::
  \boldsymbol{S}_{mi}=&
  \boldsymbol{R}_m\,\boldsymbol{R}_i\,\boldsymbol{S^F}_{mi}=S_i\,\boldsymbol{R}_m\,\boldsymbol{\hat{f}}_i
  =S_i\,\boldsymbol{\hat{f}}_{mi}\\\\
    =&\,S_i\,
     \begin{pmatrix}
...
where we have introduced the unit vector :math:`\boldsymbol{\hat{f}}_{mi}` and where the azimut angle is
