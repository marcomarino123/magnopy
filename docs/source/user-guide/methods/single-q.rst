...
and subsequently rotating them first with the rotation :math:`\boldsymbol{R_i}`, which does not
depend on the position of the unit cell in the lattice and then with the conical rotation
:math:`\boldsymbol{R_m}`, which does not depend on the position of the
atoms within unit cell:

.. include:: repeated-formulas/spin-from-ferro-any.inc

The rotation :math:`\boldsymbol{R_i}` populates each unit cell by a collection of non-collinear
atomic spins. Because :math:`\boldsymbol{R_i}` does not depend on :math:`m`, all unit
cells are replica of each other. The rotation :math:`\boldsymbol{R_m}`
generates the cone spin arrangement among unit cells.

Intra-cell spin generation
==========================
Magnopy lets each intra-cell atomic spins :math:`\boldsymbol{S_i}`
have an independent orientation, that is described by the angles
:math:`(\theta_i,\,\phi_i)` measured in the :math:`(\,u,\,v,\,n\,)` reference
frame. This is,

.. math::
   \boldsymbol{S_i} = \boldsymbol{R_i}\,\boldsymbol{S^F_i}=S_i \,\boldsymbol{\hat{f}_i}
...
================================================
The single-:math:`\boldsymbol{q}` conical spin state is described by the
unit vector :math:`\boldsymbol{\hat{n}}` that defines the cone axis direction,
and by the pitch vector :math:`\boldsymbol{q}` that defines the cone propagation.
...
.. math::
  \boldsymbol{S_{mi}}=&
  \boldsymbol{R}_m\,\boldsymbol{R_i}\,\boldsymbol{S^F_{mi}}=S_i\,\boldsymbol{R}_m\,\boldsymbol{\hat{f}_i}
